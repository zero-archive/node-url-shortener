...
var MongoProvider = function(host, port, username, password) {
    var self = this;
    ...
    this.db.open(function(db){
        if(username && password) {
            self.db.admin().authenticate(username, password, function(err, replies) {
                if(err) {
                    console.log(err);
                }
            });
...
client = new MongoProvider(host, port, config.mongo.user, config.mongo.pass);
